...
description: >-
  ...
  branches, squashing mistkit/subrepo onto v1.0.0-alpha.3, or syncing
  path-dependency CI for MistKit/BushelKit.
...
```bash
cd <container-root>   # e.g. .../BushelCloud
git fetch origin
...
2. **Primary workflow** — release branch may already have `setup-mistkit`. Add
   only where missing:
   - `env`: `MISTKIT_BRANCH: v1.0.0-beta.4`
...
3. **Secondary workflows** (if missing on release branch):
   - `.github/workflows/bushel-cloud-build.yml`
   - `.github/workflows/codeql.yml`
   - `.github/actions/cloudkit-sync/action.yml` — `mistkit-branch` input +
     conditional Setup MistKit before fallback docker build

4. **Do not carry over** stale drift: source changes, Swift version
   downgrades, vendored ConfigKeyKit, etc.
...
Add setup-mistkit to build workflows (pinned to v1.0.0-beta.4) and remove
...
2. After checkout in each build job, add sed override before setup-mistkit /
   swift-build:
...
Remove the `bushelkit` pin from `Package.resolved`.
...
git add -A && git diff --stat $RELEASE_BRANCH
git commit -m "$(cat <<'EOF'
chore: wire subrepo CI branch override for BushelKit integration branch
...
Warn: subrepo sed steps require `.package(path: "../BushelKit")` in Package.swift.

---

## Rules

1. Never modify `BushelCloud.git/` directly.
...
## Replicating in other repos

Copy `.agents/skills/rebase-integration-branches/` and symlink from
`.claude/skills/rebase-integration-branches`. Update [projects.md](projects.md).
CelestraCloud uses `CelestraKit` and `CelestraCloud.yml` instead.
